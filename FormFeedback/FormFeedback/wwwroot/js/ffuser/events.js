...
// Get Rooms Id
function fGetById(id) {
    debugger
    $.ajax({
        "url": "/UserEvents/Get/" + id,
        "type": "GET",
        "dataType": "json",
        "data": { Id: id }
    }).then((result) => {
        ...
        if (result.data[0] != null) {
            ...
            debugger
            $("#eName").html('<b>Materi Program : </b>' + result.data[0].name);
            $("#ePlaceandDateName").html('<b>Tempat & Waktu : </b> APL Tower (' + result.data[0].room + '), ' + moment(result.data[0].date).format("DD/MM/YYYY"));
